...
if __name__ == '__main__':
    url = 'https://apps.game.qq.com/cmc/cross?serviceId=182&filter=channel&chanid=3998&typeids=1,2&source=web_pc&logic=or&sortby=sIdxTime&limit=15&start=0'

    html = getpage(url)
    url_r = re.compile(r'"sVID":"(.*?)"')
...
def spider_hpjy_begin(day):
    video_from = '和平精英'
    page = 0
    print('开始爬和平精英视频')
    while True:
        print('第' + str(page) + '页')
        url = 'https://apps.game.qq.com/cmc/cross?serviceId=182&filter=channel&chanid=3998&typeids=1,2&source=web_pc&logic=or&sortby=sIdxTime&limit=15&start=' + str(
            page)
        html = getpage(url)
        url_r = re.compile(r'"sVID":"(.*?)"')
        title_r = re.compile(r'"sTitle":"(.*?)"')
