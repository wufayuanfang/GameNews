...
if __name__ == '__main__':
    url = 'https://apps.game.qq.com/wmp/v3.1/?p0=1&p1=searchKeywordsList&page=1&pagesize=30&order=sIdxTime&r0=script&r1=userObj&source=video_center&type=iType&id=2'
...
def spider_cf_begin(day):
    video_from = 'cf'
    page = 1
    print('开始爬CF视频')
    while True:
        print('第' + str(page) + '页')
        url = 'https://apps.game.qq.com/wmp/v3.1/?p0=1&p1=searchKeywordsList&page=' + str(
            page) + '&pagesize=30&order=sIdxTime&r0=script&r1=userObj&source=video_center&type=iType&id=2'
        html = getpage(url)
        url_r = re.compile(r'"sUrl":"(.*?)"')
        title_r = re.compile(r'"sTitle":"(.*?)"')
        time_r = re.compile(r'"sIdxTime":"(.*?)"')
